...
export function useSquad() {
  ...
  const overseasInXI = sel.filter(p => p.c && p.c !== 'IN').length;
  // IP must be Indian if XI already has 4 overseas
  const ipMustBeIndian = overseasInXI >= 4;
  const ip1Valid = !IP1 || IP1.c === 'IN' || overseasInXI < 4;
  const ip2Valid = !IP2 || IP2.c === 'IN' || overseasInXI < 4;
  const rolesReady = !!(C && VC && IP1 && IP2 && ip1Valid && ip2Valid);
