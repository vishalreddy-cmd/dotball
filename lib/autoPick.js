...
export function autoPick(match, squads) {
  ...
  const overseasCount = () => picked.filter(p => p.c && p.c !== 'IN').length;
  ...
  function canAdd(p) {
    ...
    // Budget headroom: remaining slots must be fillable at minimum 7cr each
    const slotsAfter = 11 - picked.length - 1;
    if (slotsAfter > 0 && creditSum([...picked, p]) + slotsAfter * 7.0 > 100) return false;
    // Max 4 overseas players in XI (IPL rule)
    if (p.c && p.c !== 'IN' && overseasCount() >= 4)  return false;
  ...
  // Phase 1: fill minimums with CHEAPEST valid players to preserve budget for Phase 2
  for (const role of ['WK', 'BAT', 'BOWL', 'AR']) {
    const cheap = [...scored.filter(x => x.r === role)].sort((a, b) => (CR[a.id] ?? 7) - (CR[b.id] ?? 7));
