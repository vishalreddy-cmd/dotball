...
export default function BoardsPage() {
  ...
  const tabBtn = (id, label, activeColor) => (
    <button
      key={id}
      onClick={() => setTab(id)}
      style={{ flex: 1, padding: 9, borderRadius: 9, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 700, background: tab === id ? activeColor : 'transparent', color: tab === id ? (id === 'season' ? '#000' : '#fff') : '#7a85a0' }}
    >
      {label}
    </button>
  );
  ...
  const emptyState = (
    <div style={{ background: '#111421', border: '1px solid #1c2035', borderRadius: 14, padding: '28px 16px', textAlign: 'center', marginBottom: 14 }}>
      <div style={{ fontSize: 32, marginBottom: 8 }}>{tab === 'season' ? '🏆' : '📊'}</div>
      <div style={{ fontSize: 13, fontWeight: 700, color: '#eef0ff', marginBottom: 4 }}>
  ...
  return (
    <div style={{ padding: '12px 12px 0' }}>
      {/* Tab bar */}
      <div style={{ display: 'flex', gap: 5, marginBottom: 12, background: '#111421', padding: 4, borderRadius: 11, border: '1px solid #1c2035' }}>
      ...
      {/* Context info */}
      <div style={{ background: '#111421', border: '1px solid #1c2035', borderRadius: 14, padding: '9px 11px', marginBottom: 12, fontSize: 10, color: '#7a85a0', lineHeight: 1.7 }}>
        {tab === 'xi' && <><span style={{ color: '#818cf8', fontWeight: 600 }}>Classic XI — </span>{matchLabel && <span style={{ color: '#424960' }}>{matchLabel} · </span>}Points from batting, bowling, fielding, economy &amp; strike rate.</>}
        {tab === 'r3' && <><span style={{ color: '#06b6d4', fontWeight: 600 }}>Top 3 — </span>{matchLabel && <span style={{ color: '#424960' }}>{matchLabel} · </span>}Total runs from your 3 predicted run scorers.</>}
      ...
      {tab === 'season' && seasonLoading && (
        <div style={{ textAlign: 'center', padding: '24px 0', color: '#7a85a0', fontSize: 11 }}>Loading season data...</div>
      )}
      ...
      {!seasonLoading && (displayRows.length === 0 ? emptyState : (
        displayRows.map((u, i) => (
          <div key={u.uid || i} style={{
            ...
            background: u.isMe ? (tab === 'season' ? '#1a1400' : '#1c1040') : '#111421',
            border: u.isMe
              ? `1.5px solid ${tab === 'season' ? '#f59e0b' : '#6366f1'}`
              : i === 0 ? '1.5px solid #f5a62344' : '1px solid #1c2035',
          }}>
            <div style={{ fontSize: i < 3 ? 19 : 12, width: 26, textAlign: 'center', flexShrink: 0 }}>{rankIcon(i)}</div>
            <div style={{ width: 34, height: 34, borderRadius: '50%', background: u.isMe ? '#312e81' : '#1c2035', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: u.isMe ? '#c7d2fe' : '#7a85a0', border: u.isMe ? '2px solid #6366f1' : '1px solid #1c2035', flexShrink: 0 }}>
              {(u.name || '?').substring(0, 2).toUpperCase()}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#eef0ff', display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              {tab === 'season'
                ? <div style={{ fontSize: 10, color: '#424960' }}>{u.matches || 0} matches · <span style={{ color: '#f5a623' }}>🥇{u.gold}</span> <span style={{ color: '#9ca3af' }}>🥈{u.silver}</span> <span style={{ color: '#b45309' }}>🥉{u.bronze}</span></div>
                : <div style={{ display: 'flex', gap: 5, marginTop: 2, fontSize: 9, color: '#424960' }}>
                    <span>🥇{u.gold || 0}</span><span>🥈{u.silver || 0}</span><span>🥉{u.bronze || 0}</span>
                  </div>
              }
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 16, fontWeight: 900, color: (u.points || u.total || u.runs) ? '#eef0ff' : '#1c2035' }}>
                {tab === 'season' ? (u.total || '—') : (u.points || u.runs || '—')}
              </div>
              {(u.points || u.total || u.runs) ? <div style={{ fontSize: 8, color: '#7a85a0' }}>{tab === 'r3' ? 'runs' : 'pts'}</div> : null}
            </div>
          </div>
        ))
      ))}
