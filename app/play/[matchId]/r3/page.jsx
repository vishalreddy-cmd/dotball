  ...
  if (done) {
    ...
    return (
      <div style={{ padding: '12px 12px 0', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 19, fontWeight: 700, color: '#eef0ff' }}>Top 3 locked!</div>
        <div style={{ fontSize: 10, color: '#22c55e', marginBottom: 4 }}>Hidden until first ball</div>
        <div style={{ fontSize: 10, color: '#7a85a0', marginBottom: 16 }}>1 run = 1 point · no bonuses</div>

        {squad3.map((p, i) => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 13px', borderRadius: 13, marginBottom: 8, background: '#111421', border: `2px solid ${T[p.team]?.bg || '#6366f1'}66`, textAlign: 'left' }}>
            <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#1c2035', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#f5a623', flexShrink: 0 }}>{i + 1}</div>
            <TeamLogo team={p.team} size={34} />
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#eef0ff' }}>{p.n}</div>
        ...
        <div style={{ display: 'flex', gap: 8, paddingBottom: 12, marginTop: 4 }}>
          <button onClick={reset} style={{ flex: 1, padding: 11, borderRadius: 11, border: '1px solid #1c2035', background: 'transparent', color: '#7a85a0', cursor: 'pointer', fontSize: 12 }}>Edit</button>
    ...
    <div style={{ padding: '12px 12px 0' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 700, color: '#eef0ff' }}>{match.t1}</span>
          <span style={{ color: '#424960', fontSize: 9 }}>vs</span>
          <span style={{ fontSize: 10, fontWeight: 700, color: '#eef0ff' }}>{match.t2}</span>
      ...
      <div style={{ borderRadius: 12, padding: '10px 13px', background: '#0c1040', border: '1px solid #6366f133', marginBottom: 12 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#eef0ff', marginBottom: 2 }}>Top 3 — Predict your top 3 run scorers</div>
        <div style={{ fontSize: 10, color: '#7a85a0' }}>Pick 3 batters · max 2 from same team</div>
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                background: p ? `${T[p.team]?.bg || '#6366f1'}22` : '#111421',
                border: p ? `2px solid ${T[p.team]?.bg || '#6366f1'}66` : '1.5px dashed #1c2035',
              }}
            >
              {p ? (
                <>
                  ...
                  <div style={{ fontSize: 7, color: '#424960' }}>tap to remove</div>
                </>
              ) : (
                <div style={{ fontSize: 9, color: '#424960', paddingTop: 6 }}>Slot {i + 1}</div>
              )}
            </div>
          );
        })}
      ...
      <div style={{ display: 'flex', gap: 5, marginBottom: 7 }}>
        {['ALL', match.t1, match.t2].map(v => (
          <button key={v} onClick={() => setTeamFilter(v)} style={{ flex: 1, padding: '5px 3px', borderRadius: 9, border: 'none', cursor: 'pointer', fontSize: 10, fontWeight: 600, background: teamFilter === v ? '#06b6d4' : '#0d0f1a', color: teamFilter === v ? '#fff' : '#7a85a0' }}>{v}</button>
        ))}
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                border: isSel ? `1.5px solid ${tc?.bg || '#6366f1'}66` : '1px solid #1c2035',
                background: isSel ? `${tc?.bg || '#6366f1'}18` : '#111421',
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: '#eef0ff' }}>{p.n}</span>
              ...
              <div style={{ width: 24, height: 24, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, border: `1.5px solid ${isSel ? `${tc?.bg || '#6366f1'}88` : '#1c2035'}`, background: isSel ? tc?.bg || '#6366f1' : '#1c2035', color: isSel ? '#fff' : '#424960' }}>
                {isSel ? '✓' : '+'}
              </div>
            </div>
          );
