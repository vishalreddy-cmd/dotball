...
export default function RecapPage() {
  ...
  if (loading) return (
    <div style={{ padding: 24, textAlign: 'center' }}>
      ...
      <div style={{ fontSize: 12, color: '#9a9590', marginTop: 8 }}>Loading your recap...</div>
    </div>
  );
  ...
  return (
    <div style={{ padding: '12px 14px 24px' }}>
      {/* Match result banner */}
      {match.res ? (
        <div style={{ borderRadius: 14, overflow: 'hidden', marginBottom: 14, border: '1px solid #22c55e44', background: 'linear-gradient(135deg,#0d1a0d,#222222)' }}>
          <div style={{ background: '#22c55e22', padding: '6px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 9, color: '#5e5a56' }}>{match.day} {match.date}</span>
          </div>
          <div style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
            <TeamLogo team={match.t1} size={36} />
            <div style={{ flex: 1, textAlign: 'center' }}>
              <div style={{ fontSize: 12, fontWeight: 800, color: '#e8e6e0', marginBottom: 2 }}>
                {match.res.t1s} <span style={{ color: '#5e5a56', fontWeight: 400 }}>vs</span> {match.res.t2s}
              </div>
        ...
        <div style={{ borderRadius: 14, padding: '12px 14px', marginBottom: 14, background: '#222222', border: '1px solid #383838', textAlign: 'center' }}>
          <div style={{ fontSize: 12, color: '#9a9590' }}>Result not yet added</div>
        </div>
      )}

      {noPicks ? (
        <div style={{ textAlign: 'center', padding: '32px 16px', background: '#222222', borderRadius: 14, border: '1px solid #383838' }}>
          <div style={{ fontSize: 36, marginBottom: 8 }}>😶</div>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#e8e6e0', marginBottom: 6 }}>No squad picked</div>
          <div style={{ fontSize: 11, color: '#9a9590' }}>You didn't pick a squad for this match.</div>
        </div>
      ) : (
        <>
          {/* Rank badge */}
          {groupRank && (
            <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
              <div style={{ flex: 1, borderRadius: 12, padding: '10px 14px', background: groupRank.rank <= 3 ? '#1a1300' : '#222222', border: `1px solid ${groupRank.rank === 1 ? '#f5a62366' : groupRank.rank <= 3 ? '#f5a62333' : '#383838'}`, textAlign: 'center' }}>
                <div style={{ fontSize: 22, fontWeight: 900, color: groupRank.rank === 1 ? '#f5a623' : groupRank.rank <= 3 ? '#f5a623' : '#e8e6e0' }}>
                  {groupRank.rank === 1 ? '🥇' : groupRank.rank === 2 ? '🥈' : groupRank.rank === 3 ? '🥉' : `#${groupRank.rank}`}
                </div>
                <div style={{ fontSize: 9, color: '#9a9590', marginTop: 2 }}>of {groupRank.total} players</div>
              </div>
              {xiSquad?.points > 0 && (
                <div style={{ flex: 1, borderRadius: 12, padding: '10px 14px', background: '#0c1040', border: '1px solid #6366f133', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 9, color: '#9a9590', marginTop: 2 }}>points</div>
                </div>
              )}
            </div>
          )}
          ...
          {/* Classic XI recap */}
          {xiSquad && (
            <div style={{ background: '#222222', borderRadius: 14, border: '1px solid #383838', marginBottom: 12, overflow: 'hidden' }}>
              <div style={{ background: '#6366f111', padding: '8px 14px', borderBottom: '1px solid #383838', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ padding: '8px 10px' }}>
                {(xiSquad.players || []).map(pid => {
                  ...
                  return (
                    <div key={pid} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '5px 0', borderBottom: '1px solid #383838' }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 700, color: pts ? '#22c55e' : '#5e5a56', minWidth: 28, textAlign: 'right' }}>
                        {pts || '—'}
                      </span>
                    </div>
                  );
          ...
          {r3Squad && (
            <div style={{ background: '#222222', borderRadius: 14, border: '1px solid #383838', marginBottom: 12, overflow: 'hidden' }}>
              <div style={{ background: '#06b6d411', padding: '8px 14px', borderBottom: '1px solid #383838', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {(r3Squad.picks || []).map((pid, i) => {
                  ...
                  return (
                    <div key={pid} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '5px 0', borderBottom: '1px solid #383838' }}>
                      ...
                      <TeamLogo team={p.team} size={20} />
                      <div style={{ flex: 1, fontSize: 11, fontWeight: 600, color: '#e8e6e0' }}>{p.n}</div>
                      <span style={{ fontSize: 11, fontWeight: 700, color: r3Squad.runs ? '#22c55e' : '#5e5a56' }}>
                        {r3Squad.runs ? '—' : '—'}
                      </span>
                    </div>
                  );
      ...
      <button
        onClick={() => router.back()}
        style={{ width: '100%', padding: 12, borderRadius: 12, border: '1px solid #383838', background: 'transparent', color: '#9a9590', fontWeight: 600, fontSize: 12, cursor: 'pointer' }}
      >
        Back
      </button>
    </div>
